...
void b2PrismaticJoint_SetLimits(b2JointId jointId, float lower, float upper)
{
	...
	{
		joint.prismaticJoint.lowerTranslation = min( lower, upper );
		joint.prismaticJoint.upperTranslation = max( lower, upper );
		joint.prismaticJoint.lowerImpulse = 0.0f;
		joint.prismaticJoint.upperImpulse = 0.0f;
	}
}
...
float b2PrismaticJoint_GetSpeed(b2JointId jointId)
{
	...
	b2Vec2 vRel = ( vB + b2CrossSV( wB, rB )) - (vA + b2CrossSV( wA, rA ));
	float speed = d.dot( b2CrossSV( wA, axisA ) ) + axisA.dot( vRel );
	return speed;
}
