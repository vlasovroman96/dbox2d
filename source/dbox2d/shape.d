...
pragma(inline, true) bool b2ShouldShapesCollide(b2Filter filterA, b2Filter filterB)
{
...
pragma(inline, true) bool b2ShouldQueryCollide(b2Filter shapeFilter, b2QueryFilter queryFilter)
{
...
b2Shape* b2GetShape(b2World* world, b2ShapeId shapeId)
{
	...
	assert( shape.id == id && shape.generation == shapeId.generation );
	return shape;
}

b2ChainShape* b2GetChainShape(b2World* world, b2ChainId chainId)
{
...
void b2UpdateShapeAABBs(b2Shape* shape, b2Transform transform, b2BodyType proxyType)
{
...
b2Shape* b2CreateShapeInternal(b2World* world, b2Body* body, b2Transform transform, const(b2ShapeDef)* def, const(void)* geometry, b2ShapeType shapeType)
{
	...
	{
		assert( world.shapes[shapeId].id == B2_NULL_INDEX );
	}
	...
	{
		case b2_capsuleShape:
			shape.capsule = *cast(const(b2Capsule)*)geometry;
			break;

		case b2_circleShape:
			shape.circle = *cast(const(b2Circle)*)geometry;
			break;

		case b2_polygonShape:
			shape.polygon = *cast(const(b2Polygon)*)geometry;
			break;

		case b2_segmentShape:
			shape.segment = *cast(const(b2Segment)*)geometry;
			break;

		case b2_chainSegmentShape:
			shape.chainSegment = *cast(const(b2ChainSegment)*)geometry;
			break;
	...
	shape.userData = cast(void*)def.userData;
	...
	if ( def.isSensor )
	{
		shape.sensorIndex = cast(int)world.sensors.count;
		b2Sensor sensor = {
...
{
	B2_CHECK_DEF( def );
	assert( b2IsValidFloat( def.density ) && def.density >= 0.0f );
	assert( b2IsValidFloat( def.material.friction ) && def.material.friction >= 0.0f );
	assert( b2IsValidFloat( def.material.restitution ) && def.material.restitution >= 0.0f );
	assert( b2IsValidFloat( def.material.rollingResistance ) && def.material.rollingResistance >= 0.0f );
	assert( b2IsValidFloat( def.material.tangentSpeed ) );
...
{
	assert( b2IsValidFloat( polygon.radius ) && polygon.radius >= 0.0f );
...
void b2DestroyShapeInternal(b2World* world, b2Shape* shape, b2Body* body, bool wakeBodies)
{
	...
	if ( shape.sensorIndex != B2_NULL_INDEX )
	{
		...
		{
			b2Visitor* ref_ = &sensor.overlaps2[i];
...
b2ChainId b2CreateChain(b2BodyId bodyId, const(b2ChainDef)* def)
{
	B2_CHECK_DEF( def );
	assert( def.count >= 4 );
	assert( def.materialCount == 1 || def.materialCount == def.count );
	...
	{
		assert( world.chainShapes[chainId].id == B2_NULL_INDEX );
	}
	...
	chainShape.materials = cast(b2SurfaceMaterial*)b2Alloc( cast(int)(materialCount * b2SurfaceMaterial.sizeof) );

	for ( int i = 0; i < materialCount; ++i )
	{
		const(b2SurfaceMaterial)* material = def.materials + i;
		assert( b2IsValidFloat( material.friction ) && material.friction >= 0.0f );
		assert( b2IsValidFloat( material.restitution ) && material.restitution >= 0.0f );
		assert( b2IsValidFloat( material.rollingResistance ) && material.rollingResistance >= 0.0f );
		assert( b2IsValidFloat( material.tangentSpeed ) );
	...
	shapeDef.userData = cast(void*)def.userData;
	...
	{
		chainShape.count = n;
		chainShape.shapeIndices = cast(int*)b2Alloc( cast(int)(chainShape.count * int.sizeof) );

		b2ChainSegment chainSegment = void;
...
{
	b2Free( chain.shapeIndices, cast(int)(chain.count * int.sizeof) );
	chain.shapeIndices = null;

	b2Free( chain.materials, cast(int)(chain.materialCount * b2SurfaceMaterial.sizeof) );
...
void b2DestroyChain(b2ChainId chainId)
{
	...
	while ( *chainIdPtr != B2_NULL_INDEX )
	{
		...
		chainIdPtr = &( world.chainShapes[*chainIdPtr].nextChainId );
	}
...
b2WorldId b2Chain_GetWorld(b2ChainId chainId)
{
	...
	return b2WorldId( cast(ushort)(chainId.world0 + 1), world.generation );
}
...
b2CastOutput b2ShapeCastShape(const(b2ShapeCastInput)* input, const(b2Shape)* shape, b2Transform transform)
{
	...
	switch ( shape.type )
	{
		...
		case b2_chainSegmentShape:
		{
			...
			approximateCentroid = approximateCentroid * ( 1.0f / localInput.proxy.count );
...
b2ShapeProxy b2MakeShapeDistanceProxy(const(b2Shape)* shape)
{
	switch ( shape.type )
	{
		...
		case b2_polygonShape:
			return b2MakeProxy( shape.polygon.vertices.ptr, shape.polygon.count, shape.polygon.radius );
		case b2_segmentShape:
...
{
	b2World* world = b2GetWorld( shapeId.world0 );
	return b2WorldId( cast(ushort)(shapeId.world0 + 1), world.generation );
}
...
b2CastOutput b2Shape_RayCast(b2ShapeId shapeId, const(b2RayCastInput)* input)
{
	...
	b2RayCastInput localInput = void;
	localInput.origin = b2InvTransformPoint( transform, input.origin );
	localInput.translation = b2InvRotateVector( transform.q, input.translation );
...
{
	assert( b2IsValidFloat( density ) && density >= 0.0f );
...
{
	assert( b2IsValidFloat( friction ) && friction >= 0.0f );
...
{
	assert( b2IsValidFloat( restitution ) && restitution >= 0.0f );
...
b2Circle b2Shape_GetCircle(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld( shapeId.world0 );
	b2Shape* shape = b2GetShape( world, shapeId );
	assert( shape.type == b2_circleShape );
	return shape.circle;
}
...
b2Segment b2Shape_GetSegment(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld( shapeId.world0 );
	b2Shape* shape = b2GetShape( world, shapeId );
	assert( shape.type == b2_segmentShape );
...
b2ChainSegment b2Shape_GetChainSegment(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld( shapeId.world0 );
	b2Shape* shape = b2GetShape( world, shapeId );
	assert( shape.type == b2_chainSegmentShape );
...
b2Capsule b2Shape_GetCapsule(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld( shapeId.world0 );
	b2Shape* shape = b2GetShape( world, shapeId );
	assert( shape.type == b2_capsuleShape );
...
b2Polygon b2Shape_GetPolygon(b2ShapeId shapeId)
{
	b2World* world = b2GetWorld( shapeId.world0 );
	b2Shape* shape = b2GetShape( world, shapeId );
	assert( shape.type == b2_polygonShape );
...
{
	assert( b2IsValidFloat( restitution ) );
...
int b2Shape_GetSensorCapacity(b2ShapeId shapeId)
{
	...
	b2Sensor* sensor = b2SensorArray_Get( world.sensors, shape.sensorIndex );
	return cast(int)sensor.overlaps2.count;
}
