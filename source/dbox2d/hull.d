...
// quickhull recursion
private b2Hull b2RecurseHull(b2Vec2 p1, b2Vec2 p2, b2Vec2* ps, int count)
{
	b2Hull hull = void;
	hull.count = 0;
	...
	b2Hull hull1 = b2RecurseHull( p1, bestPoint, rightPoints.ptr, rightCount );

	// compute hull to the right of bestPoint-p2
	b2Hull hull2 = b2RecurseHull( bestPoint, p2, rightPoints.ptr, rightCount );
...
b2Hull b2ComputeHull(const(b2Vec2)* points, int count)
{
	...
	b2AABB aabb = { { float.max, float.max }, { -float.max, -float.max } };

	// Perform aggressive point welding. First point always remains.
	// Also compute the bounding box for later.
	b2Vec2[B2_MAX_POLYGON_VERTICES] ps = void;
	int n = 0;
	...
	b2Vec2[B2_MAX_POLYGON_VERTICES - 2] rightPoints = void;
	int rightCount = 0;
	...
	b2Hull hull1 = b2RecurseHull( p1, p2, rightPoints.ptr, rightCount );
	b2Hull hull2 = b2RecurseHull( p2, p1, leftPoints.ptr, leftCount );
