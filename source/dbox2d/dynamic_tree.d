...
{
	size_t size = (b2DynamicTree.sizeof + ( b2TreeNode.sizeof  * tree.nodeCapacity +
				  tree.rebuildCapacity *  int.sizeof + (b2AABB.sizeof + ( b2Vec2.sizeof  + int.sizeof))));

	return cast(int)size;
}

ulong b2DynamicTree_GetUserData(const(b2DynamicTree)* tree, int proxyId)
{
	B2_ASSERT( 0 <= proxyId && proxyId < tree.nodeCapacity );
...
b2TreeStats b2DynamicTree_Query(const(b2DynamicTree)* tree, b2AABB aabb, ulong maskBits, b2TreeQueryCallbackFcn callback, void* context)
{
	...
	int[B2_TREE_STACK_SIZE] stack = void;
	int stackCount = 0;
	...
	while ( stackCount > 0 )
	{
		...
		if ( b2AABB_Overlaps( node.aabb, aabb ) && ( node.categoryBits & maskBits ) != 0 )
		{
			if ( b2IsLeaf( node ) )
			{
				...
				bool proceed = callback( cast(int)nodeId, node.userData, context );
				result.leafVisits += 1;
...
b2TreeStats b2DynamicTree_ShapeCast(const(b2DynamicTree)* tree, const(b2ShapeCastInput)* input, ulong maskBits, b2TreeShapeCastCallbackFcn callback, void* context)
{
	...
	b2Vec2 extension = originAABB.extents();
	...
	while ( stackCount > 0 )
	{
		...
		b2Vec2 h = node.aabb.extents() + extension;
		float term1 = abs( v.dot( p1 - c ) );
		float term2 = abs_v.dot( h );
		if ( term2 < term1 )
		{
...
int b2DynamicTree_Rebuild(b2DynamicTree* tree, bool fullBuild)
{
	...
	if ( proxyCount > tree.rebuildCapacity )
	{
		...
		b2Free( tree.leafIndices, cast(int)(tree.rebuildCapacity * int.sizeof) );
		tree.leafIndices = cast(int*)b2Alloc( cast(int)(newCapacity * int.sizeof) );

static if (B2_TREE_HEURISTIC == 0) {
		b2Free( tree.leafCenters, cast(int)(tree.rebuildCapacity * b2Vec2.sizeof) );
		tree.leafCenters = cast(b2Vec2*)b2Alloc( cast(int)(newCapacity * b2Vec2.sizeof) );
} else {
		b2Free( tree.leafBoxes, tree.rebuildCapacity * b2AABB.sizeof );
		tree.leafBoxes = cast(int*)b2Alloc( cast(int)(newCapacity * b2AABB.sizeof) );
		b2Free( tree.binIndices, tree.rebuildCapacity * int.sizeof );
		tree.binIndices = cast(int*)b2Alloc( cast(int)(newCapacity * int.sizeof) );
}
