...
b2JointId b2CreateWeldJoint(b2WorldId worldId, const(b2WeldJointDef)* def)
{
	...
	joint.weldJoint.linearImpulse = b2Vec2.init;
...
float b2Joint_GetLinearSeparation(b2JointId jointId)
{
	...
	switch ( joint.type )
	{
		...
		case b2_prismaticJoint:
		{
			...
			b2Vec2 axisA = b2Vec2( 1.0f, 0.0f ).getRotated( xfA.q );
			b2Vec2 perpA = axisA.leftPerp();
			float perpendicularSeparation = abs( perpA.dot( dp ) );
			float limitSeparation = 0.0f;

			if ( prismaticJoint.enableLimit )
			{
				float translation = axisA.dot( dp );
