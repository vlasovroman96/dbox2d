...
pragma(inline, true) private b2Vec2 b2Weight2(float a1, b2Vec2 w1, float a2, b2Vec2 w2)
{
	return b2Vec2( a1 * w1.x + a2 * w2.x, a1 * w1.y + a2 * w2.y );
}

pragma(inline, true) private b2Vec2 b2Weight3(float a1, b2Vec2 w1, float a2, b2Vec2 w2, float a3, b2Vec2 w3)
{
	return b2Vec2( a1 * w1.x + a2 * w2.x + a3 * w3.x, a1 * w1.y + a2 * w2.y + a3 * w3.y );
}

pragma(inline, true) private int b2FindSupport(const(b2ShapeProxy)* proxy, b2Vec2 direction)
{
	const(b2Vec2)* points = proxy.points.ptr;
...
private b2Simplex b2MakeSimplexFromCache(const(b2SimplexCache)* cache, const(b2ShapeProxy)* proxyA, const(b2ShapeProxy)* proxyB)
{
	...
	b2SimplexVertex*[3] vertices = [ &s.v1, &s.v2, &s.v3 ];
...
private void b2MakeSimplexCache(b2SimplexCache* cache, const(b2Simplex)* simplex)
{
	cache.count = cast(ushort)simplex.count;
	const(b2SimplexVertex)*[3] vertices = [ &simplex.v1, &simplex.v2, &simplex.v3 ];
...
private void b2ComputeSimplexWitnessPoints(b2Vec2* a, b2Vec2* b, const(b2Simplex)* s)
{
	switch ( s.count )
	{
		case 1:
...
private b2Vec2 b2SolveSimplex2(b2Simplex* s)
{
	b2Vec2 w1 = s.v1.w;
	b2Vec2 w2 = s.v2.w;
	b2Vec2 e12 = w2 - w1;
...
version (none) {
pragma(inline, true) private b2Vec2 b2ComputeSimplexClosestPoint(const(b2Simplex)* s)
{
	if ( s.count == 1 )
	{
		return s.v1.w;
...
float b2_toiTime = 0, b2_toiMaxTime = 0;
...
enum b2SeparationType {
	b2_pointsType,
	b2_faceAType,
	b2_faceBType
}
alias b2_pointsType = b2SeparationType.b2_pointsType;
alias b2_faceAType = b2SeparationType.b2_faceAType;
alias b2_faceBType = b2SeparationType.b2_faceBType;
...
private b2SeparationFunction b2MakeSeparationFunction(const(b2SimplexCache)* cache, const(b2ShapeProxy)* proxyA, const(b2Sweep)* sweepA, const(b2ShapeProxy)* proxyB, const(b2Sweep)* sweepB, float t1)
{
	b2SeparationFunction f = void;
...
private float b2FindMinSeparation(const(b2SeparationFunction)* f, int* indexA, int* indexB, float t)
{
	...
	switch ( f.type )
	{
		case b2_pointsType:
		{
			...
			b2Vec2 axisB = b2InvRotateVector( xfB.q, -f.axis);
...
//
private float b2EvaluateSeparation(const(b2SeparationFunction)* f, int indexA, int indexB, float t)
{
	b2Transform xfA = b2GetSweepTransform( &f.sweepA, t );
	b2Transform xfB = b2GetSweepTransform( &f.sweepB, t );
